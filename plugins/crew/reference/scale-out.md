...
implement→review pipeline per unit, each in its own worktree. This file is the contract for
...
**disjoint in the files they touch** — that's what makes parallel worktrees safe.
...
The shipped script handles the rest: one `pipeline()` per unit (implement → review),
`isolation: 'worktree'` on the mutating stage so parallel edits never collide, structured
results, and **one branch/PR per unit**. The dispatch prompt it builds is byte-for-byte the
