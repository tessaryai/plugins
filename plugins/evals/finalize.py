#!/usr/bin/env python3
"""
finalize.py — assemble final evals/ artifacts at step 7.

Reads the v0.4 sharded pipeline under evals/pipeline/ and the per-grader files
under evals/graders/, then:

  1. Writes evals/pipeline/meta.yaml from CLI flags (version, product_hint,
     runtime). Skips if --skip-meta is passed (orchestrator wrote it already).
  2. Generates evals/report.md from the shard contents.
  3. Writes evals/.synth-lock.yaml with SHA-256 of every shard and every grader.
  4. Runs validate.py --bundle evals/ and surfaces its exit status.

Usage:
    python3 finalize.py evals/ \
        [--version 0.4.0] \
        [--product-hint "<string>"] \
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Assemble final evals/ artifacts at step 7.",
    )
    ap.add_argument("evals_dir", help="Path to the evals/ directory.")
    ...
    print(
        f"evals/ written: {len(pipeline.get('call_sites') or [])} call sites | "
        f"{len(pipeline.get('chains') or [])} chains | "
    ...
    if failed:
        print(f"WARN: {failed} grader(s) failed schema validation -- "
              f"inspect: evals/graders/<id>.yaml")
