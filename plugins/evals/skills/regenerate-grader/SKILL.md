...
- **Never forge `_meta.grounding`.** Carry the existing value forward verbatim. This skill re-authors from the shards already on disk; it does not fetch traces, so it has no standing to stamp `observed` itself. Omitting `--grounding` on `stamp-meta` does exactly the right thing.
