...
> **TL;DR (v0.25.0, current — the coverage read names the `spans` dataset; the platform retires the `observations` alias in the SAME release. REQUIRES the matching evals-platform release — this is a hard cutover with no grace period. No contract or schema change; grader contract still v9; on-disk schema still `0.16.0`.)**
