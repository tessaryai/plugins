...
> **TL;DR (v0.23.0, current — the plugin strips to its front door: `connect` + `instrument`. Local synthesis is retired; the evals-platform's observer authors and maintains the `.tessary/` bundle via draft PRs. Grader contract still v9; on-disk schema `0.15.0` → `0.16.0`.)**
...
>
> **Migration.** If you invoked `/evals:synthesize-graders` or `/evals:regenerate-grader`, stop: connect the repo on the platform (Settings → Git) and let the observer propose the bundle; regenerate individual graders from the platform UI. If you scripted `publish.py`, push the bundle to the repo's default branch instead — the platform imports on push. If you scripted `platform.py preflight`/`fetch-traces`, use `platform.py coverage` and the MCP `query_*`/`get_trace` tools. Existing bundles keep validating; nothing about the shard contents changed. If you pin the marketplace `ref`, bump it to `evals-v0.23.0`.
