...
> **TL;DR (v0.20.0, current — trace grounding is mandatory; new `/evals:instrument` skill; `--traces` removed. BREAKING for anyone scripting `synthesize-graders`. Grader contract still v9; `_meta.grounding` added additively; on-disk schema still `0.14.0`.)**
